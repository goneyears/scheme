...
(require (file "C:/Users/Mach/Files/scheme/basic/tableoperation.rkt"))
...
;test
(put "a" "b" (program-block "main" 50 66))
(define (add-prg info pos-x pos-y)
  (let ((pblock (program-block info pos-x pos-y)))
    (put info info pblock)
    (display-block pblock)))

(define (continious-string start-posn)
  (let ((thiskey (key-value (get-key-press vp))))
    (cond ((char? thiskey)   (display-string (make-posn start-posn 300) (string thiskey)))))      
  (continious-string (+ start-posn 3)))
(continious-string 200)

(define (continus icount)
  (continus (+ icount 1)))
(continus 1)